    ...
    integration(function() {
        describe('when playing a dead character', function() {
            beforeEach(function() {
                const deck = this.buildDeck('martell', [
                    'A Noble Cause',
                    'Bran Stark (Core)', 'Bran Stark (Core)'
                ]);
        ...
        describe('when playing a duplicate of a dead character', function() {
            beforeEach(function() {
                const deck = this.buildDeck('martell', [
                    'A Noble Cause',
                    'Bran Stark (Core)', 'Bran Stark (Core)', 'Bran Stark (Core)'
                ]);
                this.player1.selectDeck(deck);
                this.player2.selectDeck(deck);
                this.startGame();
                ...
                [this.character, this.dupe, this.deadCharacter] = this.player1.filterCardsByName('Bran Stark');
        ...
        describe('when playing an attachment has the same name as a dead character', function() {
            beforeEach(function() {
                const deck = this.buildDeck('martell', [
                    'A Noble Cause',
                    'Bran Stark (Core)', 'Nymeria (WotN)', 'Nymeria (FotOG)'
                ]);
                this.player1.selectDeck(deck);
                this.player2.selectDeck(deck);
                this.startGame();
