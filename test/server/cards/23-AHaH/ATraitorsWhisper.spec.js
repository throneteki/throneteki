    ...
    integration(function() {
        beforeEach(function() {
            const deck1 = this.buildDeck('greyjoy', [
                'Time of Plenty',
                'A Traitor\'s Whisper', 'Sparrows', 'Scheming Septon'
            ]);
        ...
        describe('after A Traitor\'s Whisper is used', function() {
            ...
            it('it should let the player select a card in shadows from each player', function() {
                expect(this.player2).toHavePrompt('Waiting for opponent to use A Traitor\'s Whisper');
                expect(this.player1).toHavePrompt('Select a card in each players shadow area, if able');
