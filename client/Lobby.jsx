...
class InnerLobby extends React.Component {
    ...
    render() {
        ...
        var messages = _.map(this.props.messages, message => {
            ...
            return (
                <div key={timestamp + message.user.username + (index++).toString()}>
                    <Avatar emailHash={ message.user.emailHash } float forceDefault={ message.user.settings.disableGravatar } />
        ...
        var users = _.map(this.props.users, user => {
            return (
                <div>
                    <Avatar emailHash={ user.emailHash } forceDefault={ user.settings.disableGravatar } />
                    <span>{user.name}</span>
                </div>
            );
        });
