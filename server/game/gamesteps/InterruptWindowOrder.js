const OrderableEventFunc = {
    onCardPlaced: (game, events) => {
        const groupedEvents = events.reduce((groups, event) => {
            let existing = groups.find(group => group.location === event.location && group.bottom === event.bottom && group.player === event.player && group.owner === event.card.owner);
            if(existing) {
                existing.events.push(event);
            } else {
                let title = 'Select order to ';
                const owner = event.player === event.card.owner ? 'your' : event.card.owner.name + '\'s';
                ...
                groups.push({ title, location: event.location, bottom: event.bottom, player: event.player, owner: event.card.owner, events: [event] });
            }
            return groups;
