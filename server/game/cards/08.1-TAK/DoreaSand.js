...
class DoreaSand extends DrawCard {
    setupCardAbilities() {
        this.reaction({
            ...
            target: {
                cardCondition: card => card.location === 'play area' && card !== this && card.getType() === 'character' &&
    ...
    getNumberOfSandSnakes() {
        return this.controller.getNumberOfCardsInPlay(card => card.hasTrait('Sand Snake') && card.getType() === 'character');
    }
}
