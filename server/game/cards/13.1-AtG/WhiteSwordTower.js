...
class WhiteSwordTower extends DrawCard {
    setupCardAbilities(ability) {
        ...
        this.persistentEffect({
            match: card => card.getType() === 'character' && card.controller === this.controller && card.hasTrait('Kingsguard'),
            effect: ability.effects.modifyStrength(1)
        });

        this.persistentEffect({
            condition: () => true,
            match: player => !this.controlsKingsguard(player) && this.tracker.getNumberOfPlayedEvents(player) >= 1,
    ...
    controlsKingsguard(player) {
        return player.anyCardsInPlay(card => card.getType() === 'character' && card.hasTrait('Kingsguard'));
    }
}
