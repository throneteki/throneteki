...
class Queenscrown extends DrawCard {
    ...
    handleCards(context) {
        if(context.target) {
            this.game.addMessage('{0} places {1} in {2}\'s discard pile', context.player, context.target, context.opponent);
        }
        const placedOnBottom = context.revealed.filter(card => card !== context.target);
        if(placedOnBottom.length > 0) {
            this.game.addMessage('{0} places {1} on the bottom of {2}\'s deck, in any order', context.player, placedOnBottom, context.opponent);
        }
        this.game.resolveGameAction(GameActions.simultaneously(context =>
            context.revealed.map(card => card === context.target ? GameActions.placeCard({ card, location: 'discard pile', player: context.player }) : GameActions.placeCard({ card, location: 'draw deck', bottom: true, player: context.player }))
        ), context);
    }
}
