...
class Foamdrinker extends DrawCard {
    setupCardAbilities(ability) {
        ...
        this.action({
            ...
            target: {
                cardCondition: card => card.location === 'play area' && card.getType() === 'character' && (card.hasTrait('Captain') || card.hasTrait('Raider'))
            },
            handler: context => {
                this.game.promptForIcon(context.player, this, icon => {
                    ...
                    this.game.addMessage('{0} kneels {1} to give {2} {3} icon to {4} until the end of the phase',
