...
class HiredAssassin extends DrawCard {
    setupCardAbilities() {
        this.reaction({
            ...
            handler: context => {
                this.game.addMessage('{0} uses {1} to look at a card in shadows controlled by {2}', context.player, this, context.target.controller);
    ...
    onCardSelected(player, card) {
        if(card.getType() === 'character') {
            ...
            this.game.addMessage('{0} places {1} into {2}\'s dead pile from shadows', player, card, card.owner);
        }
        return true;
    }
