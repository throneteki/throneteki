...
class StannisBaratheon extends DrawCard {
    setupCardAbilities(ability) {
        ...
        this.reaction({
            when: {
                afterChallenge: event => this.controller.canDraw() && event.challenge.isMatch({ winner: this.controller, challengeType: 'power' }) && event.challenge.attackingPlayer.getClaim() > 0
            },
            handler: context => {
