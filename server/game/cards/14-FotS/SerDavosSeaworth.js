...
class SerDavosSeaworth extends DrawCard {
    setupCardAbilities() {
        this.reaction({
            when: {
                afterChallenge: event => this.isParticipating() && event.challenge.winner === this.controller && event.challenge.loser.faction.power > 0
            },
            message: {
            ...
            handler: context => {
                this.game.resolveGameAction(
                    GameActions.discardPower(context => ({
                        ...
                        handler: thenContext => {
                            this.game.resolveGameAction(
                                GameActions.gainPower(thenContext => ({ card: thenContext.target, amount: 1 })),
                                thenContext
                            );
                        }
                    }),
