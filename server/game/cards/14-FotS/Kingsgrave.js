...
class Kingsgrave extends DrawCard {
    setupCardAbilities(ability) {
        this.reaction({
            when: {
                afterChallenge: event => event.challenge.isMatch({ loser: this.controller, challengeType: 'power', by5: true })
            },
            cost: [
    ...
    killCharacter() {
        this.game.addMessage('{0} kneels and sacrifices {1} to kill {2}', this.context.player, this, this.context.target);
        this.game.killCharacter(this.context.target);
        return true;
    }

    returnCharacterToHand() {
        this.game.addMessage('{0} kneels and sacrifices {1} to return {2} to hand', this.context.player, this, this.context.target);
