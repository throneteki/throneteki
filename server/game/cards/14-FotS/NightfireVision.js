...
class NightfireVision extends DrawCard {
    setupCardAbilities() {
        this.action({
            condition: () => this.controller.anyCardsInPlay(card => card.getType() === 'character' && card.hasTrait('R\'hllor')),
            phase: 'taxation',
            handler: context => {
    ...
    handlePlotsRevealed(event) {
        ...
        if(player.canDraw() && event.plots.some(plot => plot.controller !== player && plot.hasTrait(this.namedTrait))) {
            let numCards = player.drawCardsToHand(3).length;
            this.game.addMessage('{0} draws {1} cards for {2} due to a plot with trait {3} being revealed',
                player, numCards, this, this.namedTrait);
