...
class MaesterHarmune extends DrawCard {
    setupCardAbilities(ability) {
        this.reaction({
            when: {
                onGoldTransferred: context => context.event.from.getGameElementType() === 'player' && context.event.from !== this.controller && context.event.to === this.controller
            },
            ...
            gameAction: GameActions.drawCards(context => ({ player: context.player, amount: 1, source: this }))
                .then({
                    message: {
                        format: 'Then, {player} and {opponent} each place a card from their hand on top of their deck',
                        args: { opponent: context => context.parentContext.event.from }
                    },
                    target: {
