...
class OurBladesAreSharp extends DrawCard {
    setupCardAbilities(ability) {
        this.persistentEffect({
            location: 'any',
            condition: () => this.controller.anyCardsInPlay(card => (card.name === 'Harrenhal' || card.name === 'The Dreadfort')),
            ...
            handler: context => {
                ...
                this.game.resolveGameAction(
                    GameActions.simultaneously(
                        context.target.map(selection => GameActions.returnCardToHand({ card: selection }))
                    ),
                    context
                );
            }
