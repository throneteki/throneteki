...
class FirstRanger extends DrawCard {
    setupCardAbilities(ability) {
        ...
        this.whileAttached({
            effect: [
                ...
                ability.effects.addTrait('Ranger')
            ]
        });
        
        this.reaction({
            when: {
                afterChallenge: event => event.challenge.winner === this.controller && event.challenge.challengeType === 'military' && this.isRangerParticipatingInChallenge()
            },
            target: {
                cardCondition: card => card.location === 'play area' && card.controller === this.game.currentChallenge.loser && card.getType() === 'character' &&
                                       !card.isUnique() && card.getStrength() <= this.getNumberOfRangers(),
                gameAction: 'kill'
            },
    ...
    isRangerParticipatingInChallenge() {
        return this.controller.anyCardsInPlay(card => card.isParticipating() && card.hasTrait('Ranger'));
    }
