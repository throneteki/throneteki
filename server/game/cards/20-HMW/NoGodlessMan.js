...
class NoGodlessMan extends DrawCard {
    setupCardAbilities(ability) {
        this.reaction({
            ...
            handler: context => {
                ...
                this.game.addMessage('{0} uses {1} to draw {2} and discard {2} from {3}\'s hand',
                    context.player, this, TextHelper.count(number, 'card'), context.event.challenge.loser);
    ...
    hasParticipatingDrownedGod(player) {
        return player.anyCardsInPlay(card => card.isParticipating() &&
