...
class Harwin extends DrawCard {
    setupCardAbilities(ability) {
        this.action({
            ...
            target:  {
                cardCondition: card => card.location === 'play area' && card.controller === this.controller && card.getType() === 'character' && !card.isParticipating() && card.getKeywords().length >= 2
            },
            cost: ability.costs.discardGold(1),
            handler: context => {
                this.game.currentChallenge.addParticipantToSide(context.player, context.target);
                this.game.addMessage('{0} uses {1} to have {2} participate in the challenge on their side',
