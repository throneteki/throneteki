...
class CompelledByTheKing extends DrawCard {
    setupCardAbilities() {
        this.reaction({
            ...
            handler: context => {
                ...
                this.game.addMessage('{0} plays {1} to stand {2}', this.controller, this, context.target);
                this.game.once('onDominanceDetermined', event => this.resolveOnDominanceDetermined(event.winner, context.target));
            }
        });
    }
    ...
    resolveOnDominanceDetermined(winner, target) {
        ...
        this.game.addMessage('{0} gains 1 power on {1} because of {2}', this.controller, target, this);
    }
}
