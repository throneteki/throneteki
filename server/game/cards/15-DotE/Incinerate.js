...
class Incinerate extends DrawCard {
    setupCardAbilities() {
        this.action({
            title: 'Reduce STR',
            condition: context => context.player.hand.filter(card => card !== context.source).length > 0,
            target: {
            ...
            handler: context => {
                this.game.promptForSelect(context.player, {
                    ...
                    onSelect: (player, toDiscard) => {
                        ...
                        this.game.addMessage('{0} discards {1} to give {2}\'s {3} STR', context.player, toDiscard, context.target, strengthReduction);
