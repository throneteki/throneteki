...
class DesertRaider extends DrawCard {
            ...
            handler: context => {
                ...
                this.game.promptWithMenu(context.player, this, {
                    activePrompt: {
                        ...
                        buttons: ChallengeTypes.asButtons({ method: 'selectIcon' }).filter(button => ['intrigue', 'power'].includes(button.arg))
                    },
                    source: this
                });
            }
    ...
    selectIcon(player, icon) {
        ...
        this.game.addMessage('{0} puts {1} into play from their dead pile to have {2} gain 1 power, have it lose {3} {4} icon and have {1} gain {3} {4} icon until the end of the phase',
            this.controller, this, this.target, icon === 'intrigue' ? 'an' : 'a', icon);
