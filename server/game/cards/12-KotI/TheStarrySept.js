...
class TheStarrySept extends DrawCard {
    setupCardAbilities() {
        this.action({
            ...
            handler: context => {
                this.game.addMessage('{0} uses {1} to move 1 power to {2} and blank them until the end of the phase', context.player, this, context.target);
        ...
        this.reaction({
            when: {
                afterChallenge: event => event.challenge.isMatch({ winner: this.controller, challengeType: 'power' }) && this.canGainPower()
            },
            handler: context => {
