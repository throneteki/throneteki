...
class ForcedMarch extends PlotCard {
    setupCardAbilities() {
        const isStandingAndKneelableMilIcon = (card) => {
            return card.isMatch({ type: 'character' }) && card.hasIcon('military') && KneelCard.allow({ card });
        };
        ...
        this.whenRevealed({
            target: {
                type: 'select',
                mode: 'unlimited',
                cardCondition: (card, context) => isStandingAndKneelableMilIcon(card) && card.controller === context.player
            },
            message: '{player} uses {source} to kneel {target}',
            handler: context => {
                this.game.resolveGameAction(
                    GameActions.simultaneously(context => context.target.map(card => (
                        ...
                        handler: context => {
                            ...
                            for(let opponent of this.game.getOpponents(context.player)) {
                                let numCards = Math.min(charactersWithMilIcon, opponent.getNumberOfCardsInPlay(isStandingAndKneelableMilIcon));
                                ...
                                this.game.promptForSelect(opponent, {
                                    source: this,
                                    cardCondition: card => isStandingAndKneelableMilIcon(card) && card.controller === opponent,
                                    gameAction: 'kneel',
                                    mode: 'exactly',
                                    numCards: numCards,
