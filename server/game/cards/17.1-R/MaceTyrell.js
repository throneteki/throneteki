...
class MaceTyrell extends DrawCard {
    setupCardAbilities(ability) {
        this.reaction({
            when: {
                onCardEntersPlay: event => event.card.isFaction('tyrell') && event.card.getType() === 'character' && event.card.isUnique() &&
                    event.card.controller === this.controller && event.card.allowGameAction('gainPower')
            },
            limit: ability.limit.perPhase(1),
            cost: ability.costs.payGold(1),
            handler: context => {
                context.event.card.modifyPower(1);
                this.game.addMessage('{0} uses {1} and pays 1 gold to have {2} gain 1 power', this.controller, this, context.event.card);
            }
        });
