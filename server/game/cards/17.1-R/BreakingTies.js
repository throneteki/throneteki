...
class BreakingTies extends PlotCard {
    setupCardAbilities(ability) {
        this.action({
            ...
            cost: ability.costs.shuffleCardIntoDeck(card => card.isLoyal() && card.getType() === 'character' && !card.kneeled),
            target: {
            ...
            handler: context => {
                ...
                this.game.addMessage('{0} uses {1} and shuffles {2} into their deck to return {3} to {4}\'s hand',
                    context.player, this, context.costs.shuffleCardIntoDeckCost, context.target, context.target.owner);
            }
        });
    }
}
