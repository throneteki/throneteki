...
class RuleByDegree extends PlotCard {
    setupCardAbilities() {
        this.whenRevealed({
            target: {
                choosingPlayer: player => player.hand.length > 4 && player.hand.length === this.getHighestHandSize(),
                activePromptTitle: 'Select 4 cards to keep',
                mode: 'exactly',
                cardCondition: (card, context) => card.isMatch({ location: 'hand', controller: context.choosingPlayer }),
                numCards: 4
            },
            message: {
            ...
            handler: context => {
                this.game.resolveGameAction(
                    GameActions.simultaneously(context => [
                        ...this.getCardsToDiscard(context).map(
    ...
    getMessageFragments(context) {
        ...
        for(const [player, cards] of this.getPlayerCardMapping(context)) {
            fragments.push(
                Message.fragment('{player} discard {cards}', { player, cards })
            );
        }
        return fragments;
    }
