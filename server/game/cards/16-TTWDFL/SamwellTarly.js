            ...
            gameAction: GameActions.lookAtDeck((context) => ({
                    ...
                    handler: (thenContext) => {
                        if (thenContext.targets.cardInHand) {
                            this.game.addMessage(
                                '{0} swaps a card from hand with a card in their deck',
                                thenContext.player
                            );
                            this.game.resolveGameAction(
                                GameActions.simultaneously([
                                    GameActions.placeCard({
                                        card: thenContext.targets.cardInHand,
                                        location: 'draw deck'
                                    }),
                                    GameActions.placeCard({
                                        card: thenContext.targets.cardInDeck,
                                        location: 'hand'
                                    })
                                ])
                            );
                ...
                .then({
                    ...
                    gameAction: GameActions.simultaneously((context) =>
                        context.player.drawDeck
                            .slice(0, 4)
                            .map((card) => GameActions.placeCard({ card, location: 'draw deck' }))
                    )
                })
        });
    }
