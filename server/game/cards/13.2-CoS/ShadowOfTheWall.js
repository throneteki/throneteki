...
class ShadowOfTheWall extends DrawCard {
    setupCardAbilities() {
        this.action({
            ...
            target: {
                cardCondition: card => card.location === 'play area' && card.getType() === 'character' && card.getNumberOfIcons() <= 1,
                gameAction: 'stand'
            },
            handler: context => {
                this.controller.standCard(context.target);
                this.game.addMessage('{0} plays {1} to stand {2}', this.controller, this, context.target);
                if(this.game.getPlayers().some(player => player.activePlot && player.activePlot.hasTrait('Winter'))) {
