const { MarshalIntoShadowsCost } = require('../Constants');
...
class ReduceableGoldCost {
    ...
    canPay(context) {
        ...
        if(baseCost !== 'X') {
            ...
            return context.player.getSpendableGold({ playingType: this.playingType }) >= reducedCost;
        }
    ...
    pay(context) {
        ...
        if(hasDupe && this.playingType === 'marshal') {
            ...
            if(baseCost !== 'X') {
                ...
                context.costs.gold = Math.max(context.xValue - reduction, context.source.getMinCost());
            }
            context.game.spendGold({ amount: context.costs.gold, player: context.player, playingType: this.playingType });
...
module.exports = ReduceableGoldCost;
