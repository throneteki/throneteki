    ...
    static isMatch(challenge, matchers) {
        return (
            ...
            Matcher.containsValue(
                matchers.initiatedChallengeType,
            ...
            Matcher.containsValue(
                matchers.initiatedAgainstPlayer,
                () => challenge.initiatedAgainstPlayer
            ) &&
            ...
            Matcher.anyValue(
                matchers.attackingAlone,
                (card) =>
                    (card.isAttacking() || card.isDeclaredAsAttacker()) &&
                    challenge.attackers.length === 1
            ) &&
            Matcher.anyValue(
                matchers.defendingAlone,
                (card) => card.isDefending() && challenge.defenders.length === 1
            ) &&
            ...
            Matcher.anyValue(
                matchers.by5,
                (value) => challenge.strengthDifference >= 5 === value
            ) &&
            Matcher.anyValue(matchers.match, (func) => func(challenge)) &&
            Matcher.anyValue(
                matchers.not,
                (notProperties) => !ChallengeMatcher.isMatch(challenge, notProperties)
            ) &&
            Matcher.anyValue(matchers.or, (orProperties) =>
                ChallengeMatcher.isMatch(challenge, orProperties)
            )
        );
    }
